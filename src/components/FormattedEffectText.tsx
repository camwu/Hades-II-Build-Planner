  ...
  return (
    <span className={`${className} whitespace-pre-wrap`}>
      {lines.map((line, lineIndex) => {
          ...
          return (
            <React.Fragment key={optionalKey}>
              {parts.map((p, index) => {
                ...
                } else if (lower === 'grape juice' || lower === 'grape juices') {
                  return (
                    <span key={index} className={`inline whitespace-nowrap ${isKeywordStyle ? 'text-hades-text font-bold' : ''}`}>
                      <img 
                        src="/assets/ui/PowerDrinkDrop.webp" 
                        alt="Grape Juice" 
        ...
        const rangePattern = `[+-]?(?:\\d+(?:\\.\\d+)?-\\d+(?:\\.\\d+)?|\\d+(?:\\.\\d+)?)[.%]?(?:\\s*(?:[Ss]ec\\b\\.?|[xX]\\b))?(?:\\s*/\\s*[+-]?(?:\\d+(?:\\.\\d+)?-\\d+(?:\\.\\d+)?|\\d+(?:\\.\\d+)?)[.%]?(?:\\s*(?:[Ss]ec\\b\\.?|[xX]\\b))?)+`;
